...
public sealed class FingerprintTests
{
    ...
    [Fact]
    public void Normalize_ShouldLowercaseTrimAndRemoveAccentsPunctuation()
    {
        var normalized = _fingerprint.Normalize("  Sênior .NET - São Paulo/SP  ");
        Assert.Equal("senior net sao paulo sp", normalized);
    }
